...
export const onMessageCreate = async (message: Message, client) => {
  ...
  if (message.author.id === process.env.OWNER_ID) {
    const reactionEmoji = client.emojis.cache.get(
      config.emoji.static.custom.HeptaLove
    );
  ...
  if (message.mentions.has(client.user)) {
    const useSlashCommandsEmbed = new MessageEmbed()
      ...
      .setDescription(
        `I'm Heptagram, The open-source & multipurpose Discord bot with the goal to be the single needed bot for any server./n To inteface with me, you use slash commands. To get started and see all my commands, use \`/help\`.`
      )
      .setFooter({
