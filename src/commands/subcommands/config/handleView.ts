...
import { viewSettings } from "../../../modules/config/viewSettings";
...
export const handleView: CommandHandler = async (
  ...
  try {
    ...
    if (setting === "global") {
      const result = await viewSettings(Heptagram, guild, config);
      if (!result) {
        await interaction.editReply({
          content: "Failed to view the settings.",
        });
        return;
      }
    ...
    await interaction.editReply({
      embeds: [errorEmbedGenerator(Heptagram, "view", errorId)],
    });
  }
};
