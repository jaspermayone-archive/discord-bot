...
switch (type) {
  ...
  case "warn": return console.log(`${yellow("[WARN]")} ${timestamp} ${yellow(content)} `);
  case "error": return console.log(`${red("[ERROR]")} ${timestamp} ${red(content)} `);
  case "debug": return console.log(`${magenta("[DEBUG]")}  ${timestamp} ${magenta(content)} `);
  case "ready": return console.log(`${green("[READY]")}  ${timestamp} ${green(content)}`);
  case "mongo": return console.log(`${cyan("[MONGO]")} ${timestamp} ${cyan(content)}`);
